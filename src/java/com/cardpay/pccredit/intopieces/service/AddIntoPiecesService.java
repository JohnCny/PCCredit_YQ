...
import com.cardpay.pccredit.common.Base64Util;
...
public class AddIntoPiecesService {
	...
	public void importExcelSupple(MultipartFile file,String productId, String customerId,String appId) {
		// TODO Auto-generated method stub
			...
			/*	for(String str : sheet){
				 ...
				 CustomerSqInfo CustomerSqInfo=readExcel.CustomerSqInfo;
				 //查找该客户（选择产品后）的调查模板上传最大次数
				 int count=InfoService.selectMaxIcount(productId, customerId);
				 CustomerSqInfo.setIcount(count+1);
				 CustomerSqInfo.setId(IDGenerator.generateID());
	...
	public void wordImport(MultipartFile file, String productId,
		...
		String fileName = map.get("fileName");
		String url = map.get("url");
		LocalExcelForm localImage = new LocalExcelForm();
		localImage.setProductId(productId);
		localImage.setCustomerId(customerId);
		localImage.setTime(new Date());
		localImage.setApplicationId(applicationId);
		if (StringUtils.trimToNull(url) != null) {
			localImage.setUri(url);
		}
		...
		localExcelDao.insertWord(localImage);
	}
	...
	public LocalExcel findByApplication(@Param("icount") Integer icount,@Param("pid") String pid,@Param("cid") String cid){
		return localExcelDao.findByApplication(icount, pid, cid);
	}
	public List<LocalExcel> findAllTimeById(@Param("pid") String pid,@Param("cid") String cid){
		return localExcelDao.findAllTimeById(pid, cid);

	}
	public LocalExcel findLocalEXcelByApplication(@Param("applicationId") String applicationId){
		return localExcelDao.findByApplication1(applicationId);
	}
	...
	public int selectAllImageByPcIdCount(@Param("pid") String pid,@Param("cid") String cid){
		return localImageDao.selectAllImageByPcIdCount(pid, cid);
		
	}
	public int selectBycardIdCount(@Param("id") String id){
		return localImageDao.selectBycardIdCount(id);
	 ...
	 * @throws IOException 
	 */
	/*public QueryResult<ImageMore> selectAllImageByPcId(ImageMore ImageMore) throws IOException, SftpException{
		List<ImageMore>result=localImageDao.selectAllImageByPcId(ImageMore);
		 List<ImageMore> list=SFTPUtil.TestImageBinary(result);
		 int size=localImageDao.selectAllImageByPcIdCount(ImageMore.getPid(), ImageMore.getCid());
		 QueryResult<ImageMore> qr = new QueryResult<ImageMore>(size,list);
		return qr;
	 ...
	 * @throws IOException 
	 */
/*	public QueryResult<ImageMore> selectBycardId(ImageMore ImageMore) throws IOException, SftpException{
		List<ImageMore>result=localImageDao.selectBycardId(ImageMore);
		 List<ImageMore> list=SFTPUtil.TestImageBinary(result);
		 int size=localImageDao.selectBycardIdCount(ImageMore.getPid());
		 QueryResult<ImageMore> qr = new QueryResult<ImageMore>(size,list);
		return qr;
